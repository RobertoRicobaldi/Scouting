...
if "NOMBRE COMPLETO / APODO" in df.columns and "NOMBRE COMPLETO / APODO" in df_fotos.columns:
    if "FOTOS JUGADORAS" in df_fotos.columns:
        df_fotos = df_fotos.rename(columns={"FOTOS JUGADORAS": "FOTO"})
...
if pagina == "Scouting":
    ...
    if not valoraciones.empty:
        top_valoradas = valoraciones.groupby("Nombre").agg(Media=("Valoración", "mean"), Cuenta=("Valoración", "count")).sort_values(by="Media", ascending=False).head(20).reset_index()
